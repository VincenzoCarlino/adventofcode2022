...
fn main() {
    ...
    let result = get_count_of_pair_overlaps(&mut reader);
    println!("{}", result);
}
...
fn get_count_of_pair_overlaps(reader: &mut impl BufRead) -> u32 {
    reader.lines().fold(0u32, |mut acc, l| {
        ...
        let there_are_overlaps = check_if_there_are_overlaps(&pairs);
...
fn check_if_there_are_overlaps(ranges: &[(u8, u8)]) -> bool {
    // Prepare input.
...
mod tests {
    use crate::{
        check_if_there_are_overlaps, get_count_of_pair_overlaps, sort_input_for_compare_pairs,
        Point, PointType,
    };
    use rstest::rstest;
    use std::fs::File;
    ...
    fn should_find_overlap(#[case] input: &[(u8, u8)], #[case] expected: bool) {
        assert_eq!(expected, check_if_there_are_overlaps(input))
    }
    ...
    fn should_get_count_of_pair_overlaps(#[case] path: &str, #[case] expected: u32) {
        ...
        assert_eq!(expected, get_count_of_pair_overlaps(&mut reader));
    }
